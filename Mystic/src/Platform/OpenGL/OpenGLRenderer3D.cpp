...
namespace Mystic
{
	Mystic::OpenGLRenderer3D::OpenGLRenderer3D()
	{
	...
	bool Mystic::OpenGLRenderer3D::API_CreateWindow(uint32_t width, uint32_t height, std::string title)
	{
	...
	void Mystic::OpenGLRenderer3D::ClearScreen()
	{
        glClearColor(0.2f, 0.3f, 0.3f, 1.0f);
        glClear(GL_COLOR_BUFFER_BIT);
	}

	bool Mystic::OpenGLRenderer3D::GetKeyState(int16_t key) const
	{
		return false;
	}

	bool Mystic::OpenGLRenderer3D::GetMouseButtonState(int8_t button) const
	{
		return false;
	}

	void Mystic::OpenGLRenderer3D::SwapBuffers()
	{
	}

	void Mystic::OpenGLRenderer3D::PollEvents()
	{
		glfwPollEvents();
	}

	void Mystic::OpenGLRenderer3D::SetFramerateLimit(uint32_t limit)
	{
	}

	void OpenGLRenderer3D::UseShaderProgram()
	{
        _shader->Use();
	}

	void OpenGLRenderer3D::SetProjectionMatrix(glm::mat4& projectionMatrix)
	{
        glUniformMatrix4fv(glGetUniformLocation(_shader->id, "projection"), 1, GL_FALSE, glm::value_ptr(projectionMatrix));
	}

	void OpenGLRenderer3D::SetViewMatrix(glm::mat4& viewMatrix)
	{
        glUniformMatrix4fv(glGetUniformLocation(_shader->id, "view"), 1, GL_FALSE, glm::value_ptr(viewMatrix));
	}

	void OpenGLRenderer3D::RenderEnt(std::string& meshKey, glm::mat4& modelMat)
	{
        _jobs.emplace_back(std::make_shared<RenderJob>(meshKey, modelMat));
	}

	void OpenGLRenderer3D::HandleJobs()
	{
	...
	uint32_t Mystic::OpenGLRenderer3D::GetTextureHandle() const
	{
		return NULL;
	}

    //TODO: Update this to use another parameter for vertices, or load vertices from a file.
	Ref<Mesh> OpenGLRenderer3D::CreateMesh(std::string& key)
	{
